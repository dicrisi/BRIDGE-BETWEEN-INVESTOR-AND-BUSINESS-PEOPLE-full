<?php
 ...
 include_once("header.php"); 
  ...
  if(isset($_POST['login']))
	  {
	$query ="select * from  businesspeople where Uid='".$_POST['email_id']."'  and	Pwd ='".$_POST['password']."'";
	$result = mysql_query($query);
	if(mysql_num_rows($result))
		{
	$row = mysql_fetch_assoc($result);
		$_SESSION['login_user'] = $row['Uid'];
		echo '<script> alert("Login success"); window.location.href = "peoplehome.php" </script>';
		}
 else
	 {
...
input[type=button], input[type=submit], input[type=reset] {
  ...
  <a href="register.php"><font Color="#0000FF">New User?</font></a>
